...
const todayReviewPlan = computed(() => {
  const unmastered = store.errors.filter(e => !e.is_mastered)
  // 按加入时间排序（最早加入的优先复习）
  const sorted = [...unmastered].sort((a, b) => {
    const aDate = new Date(a.added_at || Date.now()).getTime()
    const bDate = new Date(b.added_at || Date.now()).getTime()
    return aDate - bDate
  })
  return sorted.slice(0, 5).map(e => {
    const ml = e.mastery_level || 3
    let stage
    if (ml >= 4) stage = { label: '巩固中', class: 'consolidating' }
    else {
      try {
        const added = new Date(e.added_at || Date.now()).getTime()
        const days = Math.floor((Date.now() - added) / 86400000)
        if (days <= 3) stage = { label: '新错', class: 'new' }
        else stage = { label: '理解中', class: 'understanding' }
      } catch {
        stage = { label: '理解中', class: 'understanding' }
      }
    }
    return {
      error: e,
      topic: truncateText(e.display_question || e.question || '', 30),
      stage,
      lastReviewText: getLastReviewText(e)
    }
  })
})
...
/* ============================================================
 * Future Review Plan (间隔重复时间线)
 *
 * 当前无后端 SRS 算法，使用前端规则生成近似时间线。
 * 需要后端提供 spaced_repetition 接口以支持精确排期。
 * ============================================================ */
const reviewSchedule = computed(() => {
  const unmastered = store.errors.filter(e => !e.is_mastered)
  const mastered = store.errors.filter(e => e.is_mastered)

  const today = new Date()
  const schedule = [
    { label: '今天', offset: 0, tone: 'urgent' },
    { label: '明天', offset: 1, tone: 'soon' },
    { label: '3 天后', offset: 3, tone: 'normal' },
    { label: '7 天后', offset: 7, tone: 'normal' }
  ]

  return schedule.map(slot => {
    const tasks = []
    const slotDate = new Date(today)
    slotDate.setDate(slotDate.getDate() + slot.offset)

    // 未掌握的错题：今天和明天安排复习
    if (slot.offset === 0) {
      unmastered.slice(0, 3).forEach(e => {
        const cats = (e.categories || []).join('、')
        tasks.push(`${cats} · 第 1 次复习`)
      })
    } else if (slot.offset === 1) {
      unmastered.slice(3, 6).forEach(e => {
        const cats = (e.categories || []).join('、')
        tasks.push(`${cats} · 第 1 次复习`)
      })
    } else if (slot.offset === 3) {
      // 3 天后：巩固中未掌握 + 部分已掌握需要检测
      const consolidating = unmastered.filter(e => (e.mastery_level || 3) >= 4)
      consolidating.slice(0, 2).forEach(e => {
        const cats = (e.categories || []).join('、')
        tasks.push(`${cats} · 掌握检测`)
      })
      // 如果没有巩固中，从理解中选
      if (tasks.length === 0) {
        unmastered.slice(0, 2).forEach(e => {
          const cats = (e.categories || []).join('、')
          tasks.push(`${cats} · 巩固复习`)
        })
      }
    } else if (slot.offset === 7) {
      // 7 天后：已掌握的错题进行长期记忆检测
      mastered.slice(0, 2).forEach(e => {
        const cats = (e.categories || []).join('、')
        tasks.push(`${cats} · 长期记忆检测`)
      })
      if (tasks.length === 0) {
        tasks.push('暂无已掌握错题需要检测')
      }
    }
...
function handleTrainClick() {
  ElMessage.info('专项训练功能需要后端 AI 支持，即将开放')
}
...
function startDailyReview() {
  ...
  if (unmastered.length === 0) {
    const first = store.filteredErrors[0]
    if (first) openDetailForItem(first)
  } else {
    openDetailForItem(unmastered[0])
  }
}
