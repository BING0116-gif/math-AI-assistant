...
const modeName = (mode) => mode === 'assessment' ? '智能检测' : '专项练习'
const openRow = (row) => router.push(row.resume_path || row.result_path)
</script>
<template>
  <AppShell><template #topbar-title>学以致用</template>
    <main class="apply-hub"><p class="eyebrow">学以致用</p><h1>把理解变成掌握</h1><p>选择一个明确的范围进行巩固，或在练习完成后再进入智能检测。</p>
      <section class="mode-grid"><article><h2>专项练习</h2><p>自己选范围 · 即时反馈 · 不使用 AI</p><button @click="router.push('/apply/practice')">开始专项练习</button></article><article><h2>智能检测</h2><p>依据画像 · 自动规划 · 统一判卷</p><button @click="router.push('/apply/assessment')">开始智能检测</button></article></section>
