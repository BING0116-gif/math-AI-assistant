...
function handleKeydown(e: KeyboardEvent) {
  // 输入法组合态（如中文拼音上屏）期间不拦截，先让文字正常上屏
  if (e.isComposing || e.keyCode === 229) return
  if (e.key === 'Enter' && !e.shiftKey) {
    e.preventDefault()
    // Shift+Enter 走浏览器默认行为 → 换行；单独 Enter 才发送
    handleSend()
  }
}
...
// 事件包装：disabled 时忽略输入。注意不能在模板里写
// "disabled ? null : handleXxx" 这类三元表达式——Vue 只求值表达式，
// 返回的函数引用会被丢弃，handler 永远不会被调用。
function onKeydown(e: KeyboardEvent) {
  if (!props.disabled) handleKeydown(e)
}

function onPaste(e: ClipboardEvent) {
  if (!props.disabled) handlePaste(e)
}

function onAddImage() {
  if (!props.disabled) handleFileSelect()
}

function onDrop(e: DragEvent) {
  if (!props.disabled) handleDrop(e)
}
