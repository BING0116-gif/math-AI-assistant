...
# 向后兼容别名：旧代码从 config.prompts 导入 SYSTEM_PROMPT，
# 新代码直接使用 BASE_PROMPT_TEMPLATE 或 SystemPromptManager。
...
"""
    ...
    def get_version_history(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "version": v.version,
                "tool_hash": v.tool_hash,
                "created_at": v.created_at,
                "description": v.description,
            }
            for v in self._version_cache
        ]
    ...
    def interpolate_variables(
        ...
        """
        在 Prompt 中插值变量（用于动态上下文注入）。

        Args:
            prompt: 待处理的 Prompt 模板。
            variables: 变量名 -> 值 的映射。
