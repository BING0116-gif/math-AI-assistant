...
5. 最多进行 3 次工具调用
6. ask_student 反问工具（结构化澄清）使用规范：
   a. 仅在两种情况调用：题目缺失必要条件无法求解（如几何题缺角度/直角信息）；
      存在多种解法路径需要学生选择
   b. 每一轮回答中最多调用 1 次；调用后立即停止推理，等待学生回答，不要自己猜测补全条件
   c. 禁止用 ask_student 向学生索要答案，或让学生确认你算出的结果
   d. 题目信息完整时不要调用它（那是多余的反问）"""
