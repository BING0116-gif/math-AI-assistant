...
from app.data.models import Base, Chapter, Course, KnowledgeGraphVersion, KnowledgePoint, LearningRecord, Question, QuestionKnowledgePoint, User
from app.services.assessment_service import AssessmentBlueprintPlanner, create_assessment, get_assessment, save_draft, start_assessment, submit_assessment
from app.services.practice_service import PracticeError


class StubPlanner:
    async def plan(self, signals, inventory, question_count):
        return {"plan":{"buckets":[{"knowledge_point_code":"limit","count":question_count,"difficulty_min":1,"difficulty_max":5,"reason_code":"weak_mastery","reason":"画像显示该知识点需要复测"}]},"source":"llm","model":"stub-deepseek","latency_ms":10,"token_usage":{"total_tokens":10},"error_code":None}
...
async def test_assessment_uses_bank_persists_drafts_and_isolates_owner(monkeypatch):
    ...
    for q in session["questions"]: await save_draft("user-1",session["session_id"],q["question_id"],"A",0)
    result=await submit_assessment("user-1",session["session_id"],"submit-key")
    assert result["correct"]==5 and result["status"]=="completed"
    assert (await submit_assessment("user-1",session["session_id"],"submit-key"))["correct"]==5
    async with factory() as db:
...
async def test_assessment_planner_requests_deepseek_model(monkeypatch):
    ...
    class Response:
        content = '{"buckets":[{"knowledge_point_code":"limit","count":1,"difficulty_min":1,"difficulty_max":5,"reason_code":"coverage","reason":"范围覆盖"}]}'
        model = "deepseek-chat"
        latency_ms = 5
        usage = {}
