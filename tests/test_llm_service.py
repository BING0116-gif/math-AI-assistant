...
class TestLLMServiceDifficultyAnalysis:
    """测试难度分析"""
    ...
    @pytest.mark.asyncio
    async def test_analyze_difficulty_valid_json(self, svc):
        """测试正常 JSON 返回"""
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(
                message=MagicMock(
                    content='''```json
{"estimated_difficulty": 3, "reason": "中等难度", "knowledge_points": ["积分", "换元法"]}
```'''
                )
            )
        ]
        ...
        with patch.object(svc, '_client', mock_client):
            result = await svc.analyze_question_difficulty(
                "计算 ∫x²dx", "calculus"
            )
    ...
    @pytest.mark.asyncio
    async def test_analyze_difficulty_invalid_json(self, svc):
        """测试 JSON 解析失败时的降级"""
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(
                message=MagicMock(content="invalid json response")
            )
        ]
        mock_response.usage = None
        mock_response.model = "test-model"
        ...
        with patch.object(svc, '_client', mock_client):
            result = await svc.analyze_question_difficulty(
                "some question", "algebra"
            )
            assert result["estimated_difficulty"] == 3  # 默认值
