...
def _run_subprocess(mode: str) -> dict:
    ...
    if mode == "no-key":
        ...
        # LLM_API_KEY 现也决定 AI 可用性（文本 LLM 优先用它，兜底 DASHSCOPE_API_KEY）。
        # 必须一并清空，否则 .env 中的 LLM_API_KEY 会让子进程误判 AI 可用并触发 DB 迁移。
        env["LLM_API_KEY"] = ""  # 空字符串覆盖 .env 文件中的值
    elif mode == "disabled":
...
class TestAiCapabilityEdgeCases:
    ...
    def test_health_remains_healthy_when_ai_offline(self, monkeypatch):
        ...
        import app.services.ai_capability as cap_mod
        monkeypatch.setattr(live_settings, "DASHSCOPE_API_KEY", "")
        cap_mod.reset_for_test()
        with patch("main.agent", Mock()), \
             patch("main.registry", Mock()), \
             patch("main.error_book_manager", MagicMock()):
    ...
    def test_ai_capability_state_compute(self, monkeypatch):
        ...
        from app.config.settings import settings as live_settings
        import app.services.ai_capability as cap_mod
        monkeypatch.setattr(live_settings, "DASHSCOPE_API_KEY", "test-key")
        monkeypatch.setattr(live_settings, "AI_ENABLED", True)
        cap_mod.reset_for_test()
