...
from app.services.learning_projection import CALCULATION_VERSION, _calculate_state, policy_version


def _attempt(number, *, correct, kind="regular", days=0, difficulty=3, hint=False, solution=False, spent=None):
    return SimpleNamespace(
        id=f"attempt-{number}", user_id="projection-user", question_id=f"Q-{number}", correct=correct, user_answer="A",
...
def test_projection_is_deterministic_and_single_success_is_not_mastery():
    ...
    assert first["calculation_version"] == policy_version()
