...
from app.services.practice_service import PracticeError, complete_session, create_session, create_session_from_questions, get_session, start_session, submit_attempt
...
@pytest.mark.asyncio
async def test_create_session_from_questions_bridges_rag_ids(monkeypatch):
    engine = create_async_engine("sqlite+aiosqlite:///:memory:")
    @event.listens_for(engine.sync_engine, "connect")
    def foreign_keys(connection, _): connection.execute("PRAGMA foreign_keys=ON")
    factory = async_sessionmaker(engine, expire_on_commit=False)
    ...
    async with factory() as db:
        ...
        for index in range(3):
            ...
            db.add(question); await db.flush(); db.add(QuestionKnowledgePoint(question_id=question.id, knowledge_point_id="point-1"))
        essay = Question(id="Q-essay", content="证明题", question_type="essay", options=None, answer="证明", analysis="解析", category="导数", difficulty=4, course_id="course-1", version_id="version-1", review_status="published", grading_mode="manual", practice_eligible=True, answer_spec={"kind": "essay"})
        db.add(essay)
        await db.commit()
    session = await create_session_from_questions(
        "user-1", ["Q-2", "Q-0", "Q-essay", "Q-1"], "course-1", "version-1",
    )
    ids = [q["question_id"] for q in session["questions"]]
    assert ids == ["Q-2", "Q-0", "Q-1"]
    assert "answer_spec" not in session["questions"][0] and "analysis" not in session["questions"][0]
    assert session["config"]["selection"] == "rag"
    with pytest.raises(PracticeError):
        await create_session_from_questions("user-1", ["Q-missing"], "course-1", "version-1")
    await engine.dispose()
