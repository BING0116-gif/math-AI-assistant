...
class Question(Base):
    ...
    # Versioned, deterministic variant template definition.  Only the
    # whitelist in variant_generation.py is executable; free-form prompt text
    # is never treated as a template.
    variant_blueprint = Column(JSON, nullable=True)
    ...
    variant_generation = relationship(
        "VariantGeneration", foreign_keys="VariantGeneration.generated_question_id",
...
class VariantGeneration(Base):
    """Owner-bound provenance and validation evidence for one generated variant."""
    __tablename__ = "variant_generations"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    error_item_id = Column(Integer, ForeignKey("error_items.id", ondelete="CASCADE"), nullable=False, index=True)
    source_question_id = Column(String(20), ForeignKey("questions.id", ondelete="RESTRICT"), nullable=False, index=True)
    generated_question_id = Column(String(20), ForeignKey("questions.id", ondelete="RESTRICT"), nullable=False, unique=True)
    ...
    duplicate_fingerprint = Column(String(64), nullable=False, index=True)
...
class SourceDocument(Base):
    ...
    license_evidence_ref = Column(String(500), nullable=True)
    license_confirmed_at = Column(DateTime(timezone=True), nullable=True)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
...
class ChatSession(Base):
    ...
    tools_used = Column(String(200))
    default_tutor_mode = Column(String(30), nullable=False, default="step_by_step")
...
class AIInteractionRun(Base):
    """Structured, user-scoped AI invocation telemetry without raw prompts."""

    __tablename__ = "ai_interaction_runs"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    chat_session_id = Column(String(36), ForeignKey("chat_sessions.id", ondelete="SET NULL"), nullable=True, index=True)
    practice_session_id = Column(String(36), ForeignKey("practice_sessions.id", ondelete="SET NULL"), nullable=True, index=True)
    request_kind = Column(String(40), nullable=False, index=True)
    tutor_mode = Column(String(30), nullable=True)
...
class ErrorItem(Base):
    ...
    # ISO-8601 timestamps with timezone offsets can exceed 30 characters.
    added_at = Column(String(64), default="")
...
class OutboxEvent(Base):
    """Durable side effect emitted in the same transaction as SQL facts."""
...
class ReviewSchedule(Base):
    ...
    algorithm_version = Column(String(40), nullable=False, default="spaced-review-v1")
    last_reviewed_at = Column(DateTime(timezone=True), nullable=True)
...
class ReviewScheduleAction(Base):
    """Immutable, idempotent student actions applied to a review schedule."""
    __tablename__ = "review_schedule_actions"

    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    schedule_id = Column(Integer, ForeignKey("review_schedules.id", ondelete="CASCADE"), nullable=False)
    idempotency_key = Column(String(128), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "idempotency_key", name="uq_review_action_owner_key"),
        Index("ix_review_action_owner_schedule", "user_id", "schedule_id"),
    )


class LearningActivitySession(Base):
    """Auditable active-learning time; never inferred from message counts."""
    __tablename__ = "learning_activity_sessions"
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "client_session_id", name="uq_learning_activity_owner_client"),
        Index("ix_learning_activity_owner_started", "user_id", "started_at"),
        Index("ix_learning_activity_owner_status", "user_id", "status"),
    )


class LegacyClientImport(Base):
    """Idempotent audit record for one-time browser-state imports."""
    __tablename__ = "legacy_client_imports"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    batch_id = Column(String(128), nullable=False)
    status = Column(String(20), nullable=False, default="processing")
    imported_chats = Column(Integer, nullable=False, default=0)
    imported_errors = Column(Integer, nullable=False, default=0)
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "batch_id", name="uq_legacy_import_owner_batch"),
        Index("ix_legacy_import_owner_created", "user_id", "created_at"),
    )
...
class PracticeSessionDraftAnswer(Base):
    __tablename__ = "practice_session_draft_answers"
    ...
    __table_args__ = (
        UniqueConstraint("session_id", "session_question_id", name="uq_practice_session_draft_question"),
    )


# Import compatibility for older internal callers. New code uses the generic name.
AssessmentDraftAnswer = PracticeSessionDraftAnswer
...
class QuestionRevision(Base):
    """Immutable snapshot created before a material question change."""
    ...
    __table_args__ = (
        UniqueConstraint("question_id", "version", name="uq_question_revision_version"),
        Index("ix_question_revision_question_created", "question_id", "created_at"),
    )


class QuestionFeedback(Base):
    """Operator-submitted content issue and its auditable disposition."""
    ...
    __table_args__ = (
        Index("ix_question_feedback_question_status", "question_id", "status"),
    )
