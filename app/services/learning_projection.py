...
CALCULATION_VERSION = "uks-rules-v1"
...
def _calculate_state(rows: list[PracticeAttempt]) -> dict[str, Any]:
    ...
    return {
        ...
        "next_review_at": next_review_at, "calculation_version": CALCULATION_VERSION,
        "calculation_reason": {"rule": CALCULATION_VERSION, "latest_attempt_kind": _kind(last), "latest_correct": bool(last.correct), "latest_weight": history[-1]["weight"]},
    ...
    async with get_db_session() as db:
        ...
        for code, rows in sorted(grouped.items()):
            ...
            db.add(ReviewSchedule(
                user_id=user_id, knowledge_point_code=code, due_at=state["next_review_at"],
                interval_days=state["interval_days"], consecutive_correct=state["consecutive_correct"],
