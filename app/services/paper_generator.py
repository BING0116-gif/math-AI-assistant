...
def _expr_equiv(student: str, canonical: Any) -> bool:
    try:
        import sympy
        a = sympy.sympify(str(student).strip())
        b = sympy.sympify(str(canonical).strip())
...
def _grade_one(snapshot: Dict[str, Any], answer: Any) -> Dict[str, Any]:
    ...
    if kind == "choice":
        ...
        correct = _expr_equiv(str(answer or ""), spec.get("canonical"))
    return {
        "correct": correct,
        "correct_answer": {
